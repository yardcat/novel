...
func NewGinRouter(conn *grpc.ClientConn) *gin.Engine {
	...
	cardRouterGroup := r.Group("/card")
	{
		...
		cardRouterGroup.POST("/send_cards", c.SendCard)
		cardRouterGroup.POST("/discard_cards", c.DiscardCards)
		cardRouterGroup.POST("/show_draw_cards", c.ShowDrawCards)
		cardRouterGroup.POST("/show_discard_cards", c.ShowDiscardCards)
		cardRouterGroup.POST("/show_exhaust_cards", c.ShowExhaustCards)
