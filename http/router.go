...
func NewGinRouter(conn *grpc.ClientConn) *gin.Engine {
	...
	cardRouterGroup := r.Group("/card")
	{
		...
		cardRouterGroup.POST("/show_draw_cards", c.DiscardCards)
		cardRouterGroup.POST("/show_discard_cards", c.DiscardCards)
		cardRouterGroup.POST("/show_exhaust_cards", c.DiscardCards)
